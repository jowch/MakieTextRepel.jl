...
- **`src/crossings.jl`** — leader-line crossing repair. `find_crossings` (via `segments_cross`) detects intersecting connectors; `repair_crossings!` runs a deterministic 2-opt `swap_positions!` pass (1–3 outer iters on the fixtures, hard-capped at `max_iter = 100`) and `@warn`s if residual crossings remain at cap-out — best-effort with a backstop, never silent. Under `ProjectionSolver` it runs **mid-pipeline (before legalize)**, so the final legalize pass can re-introduce a crossing — crossings are *not* eliminated, only kept no worse than the force solver on the fixtures (the v0.3 zero-overlap guarantee replaced the v0.2 no-crossing aim).

ProjectionSolver pure pipeline stages (GeometryBasics-only; composed by `solvers/projection.jl`):

- **`src/side_select.jl`** — greedy discrete Imhof-slot refinement. `side_select` picks, per label, the in-bounds Imhof slot minimizing `overlap_count·overlap_weight + ‖offset‖` (overlap-avoidance dominates, leader length is the tiebreak), seeded from the Voronoi-informed init. Pinned labels fixed; obstacles count as fixed boxes. Deterministic.
...
- **`src/cost.jl`** — `label_cost`, a **read-only** placement-quality functional (Q). Reports three independent, never-collapsed components: `overlaps` (label-pair box penetrations > 0.5 px), `mean_leader` (mean ‖offset‖ over non-dropped labels), `crossings` (via `connector_for` + `find_crossings`). Never mutates, never feeds back into placement — diagnostics only.
...
- Deferred work is tracked as GitHub issues (#3 glyph-fallback, #4 rounded `cornerradius`, #5 `[sources]` URL flip, #8 solver-seam export). v0.2 (Voronoi-informed init + leader-line crossing repair) landed in #11 (`src/voronoi.jl`, `src/crossings.jl`). v0.3 (the `ProjectionSolver` — side-select → legalize → geometric drop, now the default; zero-overlap guarantee replacing v0.2's no-crossing aim) landed in #17 (`src/side_select.jl`, `src/legalize.jl`, `src/cost.jl`, `src/solvers/projection.jl`). Real scan-line VPSC in `legalize.jl` is still deferred (Dykstra stand-in shipped).
