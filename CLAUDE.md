...
Several pure layers feed a thin Makie recipe. Keep the layers Makie-free where they already are — it's what makes the output deterministic and unit-testable.
...
- **`src/init.jl`** — initial label placement. `initial_offsets` seeds each label at the highest-preference Imhof slot (`IMHOF_ORDER = TR > R > T > BR > L > BL > B > TL`) via `slot_offset` — the most-preferred slot that fits inside the anchor's Voronoi cell, TR fallback when none fit.
...
- **`src/crossings.jl`** — post-solve leader-line crossing repair. `find_crossings` (via `segments_cross`) detects intersecting connectors; `repair_crossings!` runs a deterministic 2-opt `swap_positions!` pass (1–3 outer iters on the fixtures, hard-capped at `max_iter = 100`) and `@warn`s if residual crossings remain at cap-out — best-effort with a backstop, never silent.
- **`src/solvers/`** — internal `AbstractClusterSolver` interface (`abstract.jl`) with a `ForceSolver` wrapping `solve_repel` (`force.jl`). A seam for future Julia-native solvers; internal, a v0.3 export candidate — not public API yet.
